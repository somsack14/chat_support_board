    ...
    var SBChat = {
        ...
        initChat: function () {
            ...
            SBF.getActiveUser(true, () => {
                ...
                if (!tickets && CHAT_SETTINGS['popup'] && !storage('popup') && (!mobile || !CHAT_SETTINGS['popup-mobile-hidden'])) {
                    this.popup();
                }
                SBChat.automations.runAll();
        ...
        populate: function () {
            if (this.conversation) {
                ...
                if (!this.dashboard) {
                    this.scrollBottom();
                    this.calculateLabelDates();
                    if ((admin && SB_ADMIN_SETTINGS['notify-email-cron']) || (!admin && CHAT_SETTINGS['notify-email-cron'])) {
        ...
        followUpCheck: function () {
            return !admin && this.conversation && CHAT_SETTINGS.follow && activeUser() && !activeUser().get('email') && SBF.storageTime('email', 24) && (CHAT_SETTINGS['office-hours'] || !CHAT_SETTINGS.follow['disable-office-hours']);
        },

        // Welcome message
        welcome: function () {
            if (!tickets && (CHAT_SETTINGS['welcome-trigger'] != 'open' || this.chat_open) && (CHAT_SETTINGS['office-hours'] || !CHAT_SETTINGS['welcome-disable-office-hours']) && CHAT_SETTINGS['welcome'] && !storage('welcome') && activeUser()) {
                SBF.ajax({
                    function: 'get-block-setting',
                    value: 'welcome'
                }, (response) => {
        ...
        offlineMessage: function () {
            if (!admin && CHAT_SETTINGS['timetable'] && (!CHAT_SETTINGS['office-hours'] || (!agents_online && !CHAT_SETTINGS['timetable-disable-agents']))) {
                ...
                switch (CHAT_SETTINGS['timetable-type']) {
                    ...
                    default:
                        setTimeout(() => {
                            if (this.conversation) {
                                let offline_message = CHAT_SETTINGS['timetable-hide'] ? `${message[0] ? `*${message[0]}*\n` : ''}${message[1]}` : '[timetable]';
                        ...
                        switch (type) {
                            ...
                            case 'registration':
                                ...
                                if (!activeUser()) {
                                    ...
                                    SBPusher.start();
                                    SBChat.initChat();
                                    if ((!CHAT_SETTINGS['init-dashboard'] || !main.find('.sb-departments-list').length) && success) {
    ...
    var SBApps = {
        ...
        dialogflow: {
                        ...
                        SBF.ajax({
                            ...
                            if (response && response[0]) {
                                if (CHAT_SETTINGS['slack-active']) {
                                    SBChat.slackMessage(activeUser().id, CHAT_SETTINGS['bot-name'], CHAT_SETTINGS['bot-image'], response[1]);
                                }
            ...
            active: function (active = true, check_dialogflow = true, check_open_ai = true) {
                ...
                let check = !admin && !SBF.cookie('sb-dialogflow-disabled') && (SBF.storageTime(name, 240) || !SBChat.agent_online) && (!CHAT_SETTINGS['dialogflow-office-hours'] || !CHAT_SETTINGS['office-hours']);
                if (check_dialogflow && check_open_ai) {
                    return (CHAT_SETTINGS['dialogflow-active'] || CHAT_SETTINGS['open-ai-active']) && check;
                }
                return (!check_dialogflow || CHAT_SETTINGS['dialogflow-active']) && (!check_open_ai || CHAT_SETTINGS['open-ai-active']) && check;
            },
                ...
                SBF.ajax({
                    ...
                    if (tickets) {
                        ...
                        if (CHAT_SETTINGS['dialogflow-disable-tickets']) {
                            ...
                            CHAT_SETTINGS['open-ai-active'] = false;
                        }
                    }
