        ...
        openAI: {
            ...
            rewrite: function (message, greetings = false, onSuccess) {
                SBF.ajax({
                    ...
                    model: SB_ADMIN_SETTINGS['open-ai-model'],
                    message: `Make the following sentence more friendly and professional '${SB_LANGUAGE_CODES[SB_ADMIN_SETTINGS['active-agent-language']]} language${greetings ? ', add greetings' : ''}: """'${message.replace('"', '\'')}"""`
                }, (response) => {
                    if (!response[0]) console.error('OpenAI: ' + JSON.stringify(response[1]));
                    onSuccess(response);
                });
            },
                ...
                getHTML: function (url, onSuccess) {
                    ...
                    SBF.ajax({ function: 'get-html', url: url }, function (response) {
                        ...
                        if (response[1] == 200) {
                            ...
                            response = response.substring(response.indexOf('<body'), response.indexOf('</body')).replace('<br>', '\n').replace('<br />', '\n').replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, '').replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/src=/g, 'data-src=').replace(/<link /g, '<');
                ...
                if (conversation_id != -1) {
                    activeUser().getFullConversation(conversation_id, (response) => {
                        ...
                        if (SBAdmin.must_translate) {
                            ...
                            if (strings.length) {
                                SBApps.dialogflow.translate(strings, SB_ADMIN_SETTINGS['active-agent-language'], (response_translate) => {
                        ...
                        // Suggestions
                        if (SB_ADMIN_SETTINGS['smart-reply'] && !SBAdmin.must_translate) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i]['conversation_id'])) {
                                ...
                                if (!active_conversation || !conversation) {
                                    if (status == 2 || (SB_ADMIN_SETTINGS['order-by-date'] && (status == 0 || status == 1))) {
    ...
    $(document).ready(function () {
        ...
        */
        ...
        $(document).on('SBNewMessagesReceived', function (e, response) {
            ...
            } else if (SB_ADMIN_SETTINGS['smart-reply']) {
